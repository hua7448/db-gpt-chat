...
const Playground: NextPage<PlaygroundProps> = ({ variant = 'full' }) => {
  ...
  // Fetch Data Sources
  const { data: dataSources, loading: loadingSources } = useRequest(async () => {
    try {
  ...
  const performStart = async (
    ...
    // Shallow-write the conversation id into the current page URL once the
    // send is committed. Keeping router.pathname here is required for the
    // standalone /lishui/chat page; writing '/' would remount the full home.
    if (router.query.id !== currentConvId) {
      router.replace({ pathname: router.pathname, query: { ...(router.query ?? {}), id: currentConvId } }, undefined, {
        shallow: true,
      });
    }
  ...
  const handleExampleClick = async (example: (typeof EXAMPLE_CARDS)[number]) => {
    ...
    try {
      ...
      if (example.dbName && (loadingSources || !matchedDb)) {
        message.warning(
          loadingSources
            ? '数据源仍在加载，请稍后再试'
            : `该常用问题需要先配置 ${example.dbName} 数据源，请先在数据库面板中添加`,
        );
        return;
      }
                ...
                <div className='mt-10 w-full'>
                  <div className='flex items-center justify-center gap-2 mb-4'>
                    ...
                    <span className='text-xs font-medium text-gray-400 dark:text-gray-500 tracking-wider uppercase'>
                      {t(isLishui ? 'common_questions' : 'recommend_examples')}
                    </span>
