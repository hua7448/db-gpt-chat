...
这份规范用于维护 DB-GPT 的下游定制版：持续吸收官方修复和能力，同时让业务改动可审计、可测试、可回滚。它是仓库协作约定，不替代官方发布说明、许可证或生产变更流程。
...
- 官方版本保持官方号，例如 `0.9.0`。
- 仅有下游兼容补丁时可使用 PEP 440 的 `0.9.0.post1`；下游新增功能可使用连续的 .postN，或在产品有明确版本体系时采用独立大版本。不要伪装成官方发布号。
- 统一修改版本时运行：
