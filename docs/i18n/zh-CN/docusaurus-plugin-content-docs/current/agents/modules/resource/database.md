...
- `DatasourceResource`: A resource that can be used to connect to various data sources in K-ICS.
It just works when you run your agent in the K-ICS environment(running in the K-ICS webserver).

In previous sections [Agents With Database](../../introduction/database), we have introduced 
how to use the database resource in the K-ICS agent, so you can refer to it for more details.
