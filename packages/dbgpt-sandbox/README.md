...
背景：AI Agent 逐步成为使用 AI 解决真实环境中各类问题的有力工具，然而真实环境的任务隔离性和安全性是企业落地中必然要考虑的问题。 K-ICS Agent 目前不支持统一、可扩展的安全沙箱环境。

#### 预期目标：

 为 K-ICS Agent 实现一个安全的沙箱执行环境（支持 Agent、工具的运行和多语言代码的执行）。 分三个部分：

1. 基于 K-ICS Agent + Docker 容器实现安全的代码执行环境，支持 Python、Shell、Node.js 等代码的执行，改造 K-ICS 现有的代码执行智能体。
