...
- **人员查询模板（重要）：当要查询"某人"（姓名/编号）的信息时，必须按此步骤：①先用一条精简 SQL 按姓名/身份证定位人员编号——SELECT aac001, aac003, aab301 FROM AC01 WHERE aac003='姓名' AND ROWNUM <= 5，只查这几个字段；②拿到人员编号后，再对每张业务表（DC03/DC04/DC05/ZD11 等）各发一条带 WHERE aac001='该编号' 的查询，每次只查该表 5~8 个关键字段；③严禁把多张表的所有列一次性 SELECT 出来，严禁一次查询拉全表全部列——每次 SQL 字段数必须控制在 10 列以内、语句长度控制在 1500 字符以内**
