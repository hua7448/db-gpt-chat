...
- **主动澄清（重要，凡“可能改变结论”的歧义，先问再动手）**：遇到下列情形，必须先用 question 工具向用户提问、拿到答复后再执行——① 业务词可能对应多张表或多套口径（如“帮扶”“重点人群”“失业人员”“新登记”“困难人员”）；② 统计口径未定（是否只算“当前有效 AAE100='1'”、按记录数还是去重人数、时间范围与基准日）；③ 统计范围未定（全市 / 某区县 / 指定机构，“某类人员”如何界定）
...
- **通用字段说明（重要）：AAC001=人员编号、AAC002=身份证号、AAE100 是各表的有效标识（多数表 '1'=有效、'0'=失效），直接使用不要臆造拼写（如 AA100/AA1000）。【注意：各表 AAE100 语义并不统一——ZD11 的该字段只对“就业困难人员”一类有值，AB01 有 1/0/2/3/9 多个取值，遇 ZD11 必须先读下方例外条款】**
...
- **现状统计口径（重要）：凡跨表关联（JOIN / EXISTS / NOT EXISTS 子查询）计数"当前有效"时，对业务表清单中出现的【每一张】业务表都要各自过滤 AAE100='1'，不能只过滤主表。例：困难认定 DC05 与失业登记 DC04 关联时，须同时 DC05.AAE100='1' AND DC04.AAE100='1'；DC05 与就业登记 DC03 关联同理。【但 ZD11 是例外，见下条】**
- **【ZD11 例外·AAE100（重要，2026-09-15 实测确认，勿回退）】ZD11（重点群体帮扶人员信息清洗表）的 AAE100 是"就业困难人员认定有效标识"，【只对 AZD110='02'（就业困难人员）的记录有值】，其余人员类别（01 失业人员 / 03 离校未就业高校毕业生 / 04 低保 / 05 低边 / 06 低收入农户）该字段为空——实测全表 52,479 行中 AAE100='1' 仅 1,903 行（3.6%）。因此：【禁止】用 AAE100 过滤 ZD11 的人群范围，否则会把其余约 96% 的重点群体整体排除（52,116 人 → 1,891 人）。统计 ZD11 时按 AZD110 的人员类别取值区分人群；"重点群体/重点人群"的总量以 ZD11 全表为准（不过滤 AAE100）。若题目明确问的是"就业困难人员（且认定有效）"，才使用 AZD110='02' AND AAE100='1'。ZD11 中表示记录已注销的是 AZD115（是否已注销，1=是 0=否）及 BAE012/BDC056，不是 AAE100。**
- **排除统计口径（重要）：统计"没有做过 X 的人/记录"时，用 NOT EXISTS (SELECT 1 FROM X表 WHERE X表.AAC001=主表.AAC001 AND X表.AAE100='1') 排除，或 LEFT JOIN + 对方表字段 IS NULL，不要用总数相减等近似算法。【其中 X 表为 ZD11 时，按上条 ZD11 例外不叠加 AAE100，否则会把"在 ZD11 中的人"误判为"不在"】；**【排除判断须以"人员"为单位】**——对 ZD11 这类"一人多行"的表，要判断该人是否存在【任意一条】记录（按 AAC001 关联，例如 EXISTS (SELECT 1 FROM ZD13 s JOIN ZD11 z2 ON z2.AZD11A=s.AZD11A WHERE z2.AAC001=主表.AAC001)），不要按记录主键逐行判断，否则会把同一个人同时算进"已做"和"未做"（实测同类问题差约 120 人）**
