...
class ModelMessage(BaseModel):
    ...
    """Tool calls generated by the model (native function calling), OpenAI shape.

    Populated only on assistant messages that request tool invocations.

    """

    tool_call_id: Optional[str] = None
    """The provider-native id of the tool call this message is a result of.

    Populated only on tool-role messages.
    ...
    def _parse_assistant_message(
        ...
        if content is None:
            # Assistant messages that only request tool calls carry no content.
            pass
        ...
        # Native function calling: attach tool_calls to the assistant message.
        ...
        if tool_calls:
            ...
            if result:
                result[-1].tool_calls = normalized
            else:
                result.append(
                    ModelMessage(
                        role=ModelMessageRoleType.AI,
        ...
        for message in messages:
            if message.role == ModelMessageRoleType.HUMAN:
                ...
                if message.tool_calls:
                    # Assistant tool-call message: content may be empty/null when
                    # the model only requests tool invocations.
                    entry = {
                        "role": "assistant",
